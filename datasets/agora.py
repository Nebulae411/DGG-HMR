...
class AGORA(BASE):
    ...
    def get_raw_data(self, idx):
        ...
        if self.mode == 'infer':
            img_path = self.find_image_in_subdirs(self.split, img_name)
            if img_path is None:
    ...
    def find_image_in_subdirs(self, split, img_name):
        """Find an image under split subdirectories."""
        split_dir = os.path.join(self.dataset_path, split)
        if not os.path.exists(split_dir):
            return None
            
        direct_path = os.path.join(split_dir, img_name)
